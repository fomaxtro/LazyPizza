...
class OrderRepositoryImpl(
    ...
    override suspend fun save(order: CreateOrderParams): Result<Order, DataError> {
      return safeRemoteCall { orderRemoteDataSource.placeOrder(order.toOrderRequest()) }
          .map { it.toOrder() }
    }

    override suspend fun findAll(): Result<List<Order>, DataError> {
        return safeRemoteCall { orderRemoteDataSource.fetchAll() }
            .map { orders -> orders.map { it.toOrder() } }
    }
}